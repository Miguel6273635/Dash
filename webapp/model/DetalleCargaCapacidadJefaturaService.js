sap.ui.define([
    "mantenimiento/model/DetalleCargaCapacidadJefaturaMapper"
], function (Mapper) {
    "use strict";
    ...
    function rangeFilter(property, filters) {
        var current = Mapper.range(filters);
        return property + " ge datetime'" + odataDate(current.start) + "' and " + property + " le datetime'" + odataDate(current.end) + "'";
    }
    function ordersFilter(filters) {
        var current = Mapper.range(filters);
        return "PlannedStartDate eq datetime'" + odataDate(current.start) + "' and PlannedFinishDate eq datetime'" + odataDate(current.end) + "'";
    }
    function empty() {
        return { resources: [], assignments: [], operations: [], orders: [], materials: [], movements: [], catalogs: [], meta: { unavailableEntitySets: [] } };
    }
    ...
    function load(model, filters) {
        ...
        return Promise.all(requests.map(function (request) {
            return optional(model, request.entitySet, request.filter);
            ...
            raw.meta.filters = requests.map(function (request) {
                return { entitySet: request.entitySet, filter: request.filter || "" };
            });
