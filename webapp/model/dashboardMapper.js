...
function buildCauses(orders, causes) {
    ...
    asArray(causes)
        ...
        .forEach((cause) => {
            const orderId = String(cause.OrderId);
            // Sólo una causa primaria por OT evita duplicar los conteos.
            if (!causeByOrderId.has(orderId)) {
                causeByOrderId.set(orderId, cause.CauseText || cause.CauseCode || "Sin causa registrada");
            }
        });

    // La ausencia de una causa no debe ocultar las OT no ejecutadas. Se muestra
    // explícitamente como pendiente de clasificar, sin inventar una causa SAP.
    nonExecutedOrderIds.forEach((orderId) => {
        const label = causeByOrderId.get(orderId) || "Sin causa registrada";
...
function buildCapacity(orders, resources, assignments, operations, confirmations) {
    ...
    // QAS entrega la asignación de personal por OT aunque aún no entregue una
    // capacidad diaria validada. Estas asignaciones permiten mostrar técnicos y
    // turnos reales sin inventar horas de capacidad.
    ...
    const shiftNames = shiftKeysWithPeople.map((key) => {
        ...
        return shift ? shift.text : shiftTextFor(key);
    });
